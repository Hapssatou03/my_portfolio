          ...
          recentTransactions.map((transaction) => (
            <div 
              ...
              className={`flex items-center justify-between p-4 rounded-2xl ${
                transaction.type === 'income' ? 'bg-gradient-to-r from-red-800 to-red-700' : 'bg-gradient-to-r from-red-800 to-red-700'
              }`}
            >
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-300">{transaction.description}</p>
                </div>
              </div>
              <p className={`font-bold ${transaction.type === 'income' ? 'text-white' : 'text-white'}`}>
                {transaction.type === 'income' ? '+' : '-'}${parseFloat(transaction.amount.toString()).toFixed(0)}
              </p>
            </div>
          ))
        ) : (
